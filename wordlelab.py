"""Wordle Lab
A *Wordle* solver, *Wordle* dictionary download, dictionary utility, and strategy tester for human-style game play (computers play differently).
"""
...
class Wordlist:
    ...
    def filter_words(self, guess, result):
        ...
        test_guess = ""
        if guess == test_guess:
            print(test_guess, " in play with result", result)
        test_word = ""
        for word in self.words:
            green_space = []
            yellow_space = []
            ...
            for word_place in range(len(word)):
                if result[word_place] == "G":
                    green_space.append(word[word_place])
                else:
                    green_space.append(" ")
                if result[word_place] == "Y":
                    yellow_space.append(word[word_place])
                    yellows.append(guess[word_place])
                else:
                    yellow_space.append(" ")
                if result[word_place] == "g":
                    grey_space.append(word[word_place])
                else:
                    grey_space.append(" ")
            for guess_place in range(len(result)):
                guess_letter = guess[guess_place]
                if result[guess_place] == "G":  # must include in place
                    if guess_letter != word[guess_place]:
                        if word == test_word:
                            print("For guess", guess, "with result", result, "removing", word, "because green",
                                  guess_letter, "not", word[guess_place])
                        filtered.add(word)
                        break
                elif result[guess_place] == "Y":
                    remaining_yellow_space = yellow_space[:guess_place] + [" "] + yellow_space[guess_place + 1:]
                    if guess_letter not in (remaining_yellow_space + grey_space):  # must include
                        if word == test_word:
                            print("For guess", guess, "with result", result, "removing", word, "because yellow",
                                  guess_letter, "not in word.")
                        filtered.add(word)
                        break
                    elif guess_letter == word[guess_place]:  # but not in this place
                        if word == test_word:
                            print("For guess", guess, "with result", result, "removing", word, "because", guess_letter,
                                  "is a yellow same as", word[guess_place])
                        filtered.add(word)
                        break
                elif result[guess_place] == "g":  # can't include
                    if guess_letter not in yellows and guess_letter in (
                            grey_space + yellow_space):  # except if in the yellows
                        if word == test_word:
                            print("For guess", guess, "with result", result, "removing", word, "because grey",
                                  guess_letter, "not in", green_space, "and is in word", word)
                        filtered.add(word)
                        break
                    elif guess_letter == word[guess_place]:  # definitely not here
                        filtered.add(word)
                        break
...
class Game:
            ...
            if guess:
                ...
                for place in range(len(guess)):
                    if guess[place] != word[place]:
                        yellow_grey_space.append(word[place])
                result = ""
                for place in range(len(guess)):
                    if guess[place] == word[place]:
                        result += "G"
                        self.known_greens[place].add(guess[place])
                    elif guess[place] in yellow_grey_space:
                        result += "Y"
                        yellow_grey_space.remove(guess[place])
                        self.known_yellows[place].add(guess[place])
                    else:
                        result += "g"
                        self.known_greys[place].add(guess[place])
    ...
    def run_tests(self, algorithms=[], words=[], filename="tests.log", output=False, bookmark="", word_counting=False):
            ...
            for start_word in (
                ...
                for word in words.copy():
                    this_algorithm = algorithm + (
                        (" " + start_word) if (("c" in algorithm) and (" " not in algorithm)) else "")
                    score = len(self.auto_play(word, this_algorithm, 12))
                    if word_counting:
                        self.words_in_play.original.remove(word)
                    if not score in [1, 2, 3, 4, 5, 6]:
                        debug(word, self.history)
                    scores.append(score)
                    guesses += score
                    if output:
                        with open("output " + this_algorithm + ".txt", "a") as f:
                            print(str(self.history)[1:][:-1].replace("\'", "").replace(" ", ""), file=f)
                if len(words) > 1 or word_counting:
                    ...
                    print(this_algorithm, "| guesses |", guesses,
                          "| Average |",
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if option == "6":
            ...
            game = Game(Dictionary(["wordle_dictionary.txt", "wordle_removed.txt"]).words)
            print("Testing best algorithm with 2,315 list (" + BEST + "):")
            game.run_tests([BEST], output=True)
            game = Game(Dictionary(["wordle_dictionary.txt"]).words)
            print("Testing Start Words:")
            game.run_tests(["Gek6f2.2a.1 crane"])
            game.run_tests(["Gek6f2.2a.1 salet"])
            game.run_tests(["Gek6f2.2a.1 juicy"])
            game.run_tests(["Gek6f2.2a.1 crony"])
            game.run_tests(["Gek6f2.2a.1 clint"])
            game.run_tests(["Gek6f2.2a.1 adeiu"])
            game.run_tests(["Gek6f2.2a.1 soare"])
            game.run_tests(["Gek6f2.2a.1 arose"])
            game.run_tests(["Gek6f2.2a.1 tales"])
            game = Game(Dictionary().words)
            print("Checking Start Words:")
            ...
            print("Frequencies:", game.words_in_play.get_frequencies(0))
            print("Frequencies:", game.words_in_play.get_frequencies(1))
            print("Frequencies:", game.words_in_play.get_frequencies(2))
            print("Frequencies:", game.words_in_play.get_frequencies(3))
            print("Frequencies:", game.words_in_play.get_frequencies(4))
            game = Game(Dictionary().words)
            ...
            game.run_tests(
                ["Ga.1", "Ga.2", "Ga.3", "Ga.4", "Ga.5", "Ga1", "Ga1.1", "Ga1.2", "Ga1.3", "Ga1.4", "Ga2", "Ga2.1",
                 "Ga2.2", "Ga2.3"])
            # test duplicates for known
            game.run_tests(["Gk5", "Gk6", "Gk7", "Gk8", "Gk9", "Gk10", "Gk11"])
            # test duplicates for found
            game.run_tests(
                ["Gf.1", "Gf.2", "Gf.3", "Gf.4", "Gf.5", "Gf1", "Gf1.1", "Gf1.2", "Gf1.3", "Gf1.4", "Gf2", "Gf2.1",
                 "Gf2.2", "Gf2.3", "Gf3", "Gf3.1", "Gf3.2", "Gf4", "Gf4.1"])
            ...
            game.run_tests(
                ["Gef.1", "Gef.2", "Gef.3", "Gef.4", "Gef.5", "Gef1", "Gef1.1", "Gef1.2", "Gef1.3", "Gef1.4", "Gef2",
                 "Gef2.1", "Gef2.2", "Gef2.3", "Gef3", "Gef3.1", "Gef3.2", "Gef4", "Gef4.1"])
            # test combinaton
            game.run_tests(
                ["Gek6f.1", "Gek6f.2", "Gek6f.3", "Gek6f.4", "Gek6f.5", "Gek6f1", "Gek6f1.1", "Gek6f1.2", "Gek6f1.3",
                 "Gek6f1.4", "Gek6f2", "Gek6f2.1", "Gek6f2.2", "Gek6f2.3", "Gek6f3", "Gek6f3.1", "Gek6f3.2", "Gek6f4",
                 "Gek6f4.1"])
            # test best algorithm
            print("Testing best algorithm (" + BEST + "):")
