...
class Game:
    ...
    def get_guess_word(self, algorithm, result=""):
        ...
        if self.words_in_play.count > 0:
            if start_word and self.line == 1:
                ...
                guess = self.words_in_play.words[0]
            elif "Y" in algorithm:
                guess = self.words_in_play.sort_words("Y")[0]
            elif "R" in algorithm:
    ...
    def run_tests(self, algorithms=[], words=[], filename="tests.log", bookmark=""):
        ...
        for algorithm in algorithms:
            try:
                ...
                pass
            for start_word in (
                    reversed(self.words_in_play.original) if "-" in algorithm else self.words_in_play.original):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if option in ["2", "3"]:
            ...
            break
        if option in ["4", "5"]:
            print("\nInteractive Play.\nEnter Wordle result as a 6 letter combination of G, Y, and g.")
            if option == "4":
                game = Game(Dictionary(yesterday_filename=yesterday).words)
            else:
                game = Game(Dictionary(files=[wordle], yesterday_filename=yesterday).words)
        ...
        if option == "6":
            ...
            # game.run_tests(["Ge-ck6f1.3a.1"],filename="Ge-ck6f1.3a.1")
